...
class Collection extends \Amasty\Blog\Model\ResourceModel\Abstracts\Collection
{
    ...
    public const MULTI_STORE_FIELDS_MAP = [
        ...
        PostInterface::URL_KEY => 'main_table.url_key',
    ...
    public function addFilterByStatus(): void
    {
        $storeId = $this->storeManager->getStore()->getId();
        if ($storeId != Store::DEFAULT_STORE_ID) {
            $this->getSelect()->where(
                'noDefaultStore.status = ? OR noDefaultStore.status IS NULL AND store.status = ?',
                [PostStatus::STATUS_ENABLED]
            );
        } else {
            $this->addFieldToFilter('store.status', PostStatus::STATUS_ENABLED);
        }
    }
